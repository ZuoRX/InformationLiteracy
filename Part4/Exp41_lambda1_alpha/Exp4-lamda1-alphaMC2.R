...
MC_exp41_data<-MC_data %>% 
  data.table() %>% 
  purrr::set_names("Repeat","alpha","lamda1","rho_A","rho_I") %>% 
  .[,.(rho_A=mean(rho_A),rho_I=mean(rho_I)),by=c("alpha","lamda1")]
   
write.csv(MC_data,"Exp41_lambda1_alpha/MC2_Exp41.csv",row.names = F)
write.csv(MC_exp41_data,"Exp41_lambda1_alpha/MC2_Exp41_lamda1_alpha.csv",row.names = F)
