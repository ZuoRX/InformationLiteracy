...
remove(A_temp,add_graph,C_temp,pl,pl1)
...
system.time({
  ...
  ) %dopar% get_Exp5_MC_theta_ratio(Repeat)
  stopCluster(cl)
})

MC_exp5_data<-MC_data %>% 
  data.table() %>% 
  purrr::set_names("Repeat","ratio","theta","rho_A","rho_I","rho_Ah","rho_Ih","rho_Al","rho_Il") %>% 
  .[,.(rho_A=mean(rho_A),rho_I=mean(rho_I),
       rho_Ah=mean(rho_Ah),rho_Ih=mean(rho_Ih),
       rho_Al=mean(rho_Al),rho_Il=mean(rho_Il)),by=c("ratio","theta")] 
   
write.csv(MC_data,"Exp5_ratio_theta/MC_Exp5.csv",row.names = F)
write.csv(MC_exp5_data,"Exp5_ratio_theta/MC_Exp5_ratio_theta.csv",row.names = F)
