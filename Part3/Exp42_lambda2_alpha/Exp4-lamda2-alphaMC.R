...
MC_exp42_data<-MC_data %>% 
  data.table() %>% 
  purrr::set_names("Repeat","alpha","lamda2","rho_A","rho_I","rho_Ah","rho_Ih","rho_Al","rho_Il")  %>% 
  .[,.(rho_A=mean(rho_A),rho_I=mean(rho_I),
       rho_Ah=mean(rho_Ah),rho_Ih=mean(rho_Ih),
       rho_Al=mean(rho_Al),rho_Il=mean(rho_Il)),by=c("alpha","lamda2")] 
  
write.csv(MC_data,"Exp42_lambda2_alpha/MC_Exp42.csv",row.names = F)
write.csv(MC_exp42_data,"Exp42_lambda2_alpha/MC_Exp42_lamda2_alpha.csv",row.names = F)
