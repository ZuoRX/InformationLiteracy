...
#查看本地核数
n_core<-10 #detectCores(logical = F)
...
system.time({
  ...
  ) %dopar% get_Exp3_MC_beta_alpha(Repeat)
  stopCluster(cl)
})

MC_exp3_data<-MC_data %>% 
  data.table() %>% 
  purrr::set_names("Repeat","alpha","beta_U","rho_A","rho_I","rho_Ah","rho_Ih","rho_Al","rho_Il") %>% 
  .[,.(rho_A=mean(rho_A),rho_I=mean(rho_I),
       rho_Ah=mean(rho_Ah),rho_Ih=mean(rho_Ih),
       rho_Al=mean(rho_Al),rho_Il=mean(rho_Il)),by=c("alpha","beta_U")] 
  
 
write.csv(MC_data,"Exp3_alpha/MC_Exp3.csv",row.names = F)
write.csv(MC_exp3_data,"Exp3_alpha/MC_Exp3_beta_alpha.csv",row.names = F)
