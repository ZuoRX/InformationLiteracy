...
#查看本地核数
n_core<-12 #detectCores(logical = F)
...
system.time({
  ...
  clusterEvalQ(cl, {
    ...
    set.seed(111)
    infor<-sample(c(rep(1,N*ratio),rep(2,N*(1-ratio))),N,replace = F)
    rho_A<-1:termi
    rho_I<-1:termi
    rho_Ah<-1:termi
    rho_Ih<-1:termi
  ...
  ) %dopar% get_MC_iteration_data(Repeat)
  stopCluster(cl)
})

MC_exp2_data<-MC_data %>% 
  data.table() %>% 
  purrr::set_names("Repeat","beta_U","rho_A","rho_I","rho_Ah","rho_Ih","rho_Al","rho_Il") %>% 
  .[,.(rho_A=mean(rho_A),rho_I=mean(rho_I),
       rho_Ah=mean(rho_Ah),rho_Ih=mean(rho_Ih),
       rho_Al=mean(rho_Al),rho_Il=mean(rho_Il)),by=beta_U] 
   

write.csv(MC_data,"Exp2_lamda2/MC_04.csv",row.names = F)
write.csv(MC_exp2_data,"Exp2_lamda2/MC_lamda2_04.csv",row.names = F)
